...
public class VentaComidaPK implements Serializable {
	...
	@ManyToOne
	@JoinColumn(name = "id_venta", nullable = false, foreignKey = @ForeignKey(name = "FK_ventacomida_venta"))
	private Venta venta;
	
	@ManyToOne
	@JoinColumn(name = "id_comida", nullable = false, foreignKey = @ForeignKey(name = "FK_ventacomida_comida"))
